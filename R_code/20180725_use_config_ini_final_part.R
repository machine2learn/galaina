...
initialize_galaina_param_and_input <- function(config, messaging_b, data_df = NULL, Lambda = NULL)
{
  ...
  if (stri_isempty(input_path_directed_edges_blacklist)) {
    dir_arc_blacklist_mat <- list(NULL)
  } else {
  # if (!stri_isempty(input_path_directed_edges_blacklist)) {
  ...
  all_infer_param_ls <- list(
    ...
    input_path_directed_edges_blacklist = input_path_directed_edges_blacklist,
    dir_arc_blacklist_mat = dir_arc_blacklist_mat,
  ...
  return(list(
    pc_parameters_ls = pc_parameters_ls,
    infer_copula_param_ls = infer_copula_param_ls,
    all_infer_param_ls = all_infer_param_ls,
    post_bootstrap_processing_param_ls = post_bootstrap_processing_param_ls,
    output_path_suffstat = output_path_suffstat
  ))
}
...
if (sys.nframe() == 0) {
  ...
  all_param_ls <- initialize_galaina_param_and_input(config, messaging_b, data_df, Lambda)
